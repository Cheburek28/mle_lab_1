...
options {
    ...
    stages {
        stage('Clone github repository') {
            steps {
                cleanWs()
                bat 'chcp 65001 && git clone -b main https://github.com/Cheburek28/mle_lab_1.git'
				}
			}

        stage('Checkout repo dir') {
            steps {
                bat 'chcp 65001 && cd mle_lab_1 && dir'
				}
			}
        ...
        stage('Push'){
            steps{
                bat 'chcp 65001 && docker push cheburek28/mle_lab_1:latest'
            }
        }
	}
